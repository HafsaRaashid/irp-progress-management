...
(Notifications) is implemented on branch `feat/plan-8-notifications` (2026-09-15), not yet opened
as a PR or merged — see the row in §2a and the "next action" note below.
...
**The next action is opening the Plan 8 PR** — the branch is implemented and its full test suite is
green; it needs review and merge. After that, slice 3's remaining two plans are either not started
